  ...
  $.quiz = function(el, options) {
    ...
    base.methods = {
      ...
      finish: function() {
        ...
        $(resultsScreen).show();
        $('#quiz-results').html('Ai facut ' + score + ' din ' + numQuestions + ' intrebari bine!');

        $('#progress-circle').show();
        $('#rank-btn').show();


        var user = prompt("Introdu numele tau:");

        var procent = score * 10;

        $.ajax({
          url: `../users/${user}/${procent}`,
          method: 'post'
        });
        ...
        function css3RadialProgressBar() {
          ...
          var xvaluenow = score * 10; //Insert here a specific number (0-100) and remove the comment this var, and the above code
